...
-- Barra de título
local bar=Instance.new("Frame",f)bar.Size=UDim2.new(1,0,0,30)bar.BackgroundColor3=Color3.fromRGB(40,40,45)
Instance.new("UICorner",bar).CornerRadius=UDim.new(0,8)
local titulo=Instance.new("TextLabel",bar)titulo.Size=UDim2.new(0,150,1,0)titulo.Position=UDim2.new(0,10,0,0)titulo.BackgroundTransparency=1 titulo.Text="Shinka Hub"titulo.TextColor3=Color3.fromRGB(220,220,255)titulo.TextSize=18 titulo.Font=Enum.Font.GothamBold titulo.TextXAlignment=Enum.TextXAlignment.Left

-- Botão minimizar
local min=Instance.new("TextButton",bar)min.Size=UDim2.new(0,25,0,25)min.Position=UDim2.new(1,-60,0,2.5)min.BackgroundColor3=Color3.fromRGB(80,80,150)min.Text="−"min.TextColor3=Color3.new(1,1,1)min.Font=Enum.Font.GothamBold min.TextSize=20
min.MouseButton1Click:Connect(function()g.Enabled=false end)

-- Botão fechar
local cls=Instance.new("TextButton",bar)cls.Size=UDim2.new(0,25,0,25)cls.Position=UDim2.new(1,-30,0,2.5)cls.BackgroundColor3=Color3.fromRGB(200,50,50)cls.Text="×"cls.TextColor3=Color3.new(1,1,1)cls.Font=Enum.Font.GothamBold cls.TextSize=20
cls.MouseButton1Click:Connect(function()g:Destroy()end)

-- Botão flutuante para reabrir
local reopen=Instance.new("TextButton",g)reopen.Size=UDim2.new(0,50,0,50)reopen.Position=UDim2.new(0,10,0.5,-25)reopen.BackgroundColor3=Color3.fromRGB(100,0,255)reopen.Text="SH"reopen.TextColor3=Color3.new(1,1,1)reopen.Font=Enum.Font.GothamBold reopen.TextSize=20
reopen.Visible=false
reopen.MouseButton1Click:Connect(function()g.Enabled=true reopen.Visible=false end)
...
local cont=Instance.new("Frame",area)cont.Size=UDim2.new(1,-20,1,-20)cont.Position=UDim2.new(0,10,0,10)cont.BackgroundTransparency=1

-- Função auxiliar (igual)
local function novobtn(pai,p,txt,cor,f)local b=Instance.new("TextButton",pai)b.Size=UDim2.new(0.9,0,0,35)b.Position=UDim2.new(0.05,0,p,0)b.BackgroundColor3=cor or Color3.fromRGB(60,60,70)b.Text=txt b.TextColor3=Color3.new(1,1,1)b.Font=Enum.Font.GothamSemibold b.TextSize=14
Instance.new("UICorner",b).CornerRadius=UDim.new(0,6)if f then b.MouseButton1Click:Connect(f)end return b end

-- Movement
local m=Instance.new("Frame",cont)m.Size=UDim2.new(1,0,1,0)m.BackgroundTransparency=1
...
local fly=false local flyBtn=novobtn(m,70,"Fly: OFF",nil,function()fly=not fly flyBtn.Text="Fly: "..(fly and"ON"or"OFF")flyBtn.BackgroundColor3=fly and Color3.fromRGB(0,100,50)or Color3.fromRGB(60,60,70)end)
local noclip=false local noclipConn local noclipBtn=novobtn(m,110,"Noclip: OFF",nil,function()noclip=not noclip noclipBtn.Text="Noclip: "..(noclip and"ON"or"OFF")noclipBtn.BackgroundColor3=noclip and Color3.fromRGB(0,100,50)or Color3.fromRGB(60,60,70)
...
-- Visuals
local v=Instance.new("Frame",cont)v.Size=UDim2.new(1,0,1,0)v.BackgroundTransparency=1 v.Visible=false
local origBright,origFog,origShadow=l.Brightness,l.FogEnd,l.GlobalShadows
local fb=false local fbBtn=novobtn(v,10,"Fullbright: OFF",nil,function()fb=not fb if fb then l.Brightness=2 l.GlobalShadows=false l.Ambient=Color3.new(1,1,1)else l.Brightness=origBright l.GlobalShadows=origShadow l.Ambient=Color3.new(0,0,0)end fbBtn.Text="Fullbright: "..(fb and"ON"or"OFF")fbBtn.BackgroundColor3=fb and Color3.fromRGB(0,100,50)or Color3.fromRGB(60,60,70)end)
local nf=false local nfBtn=novobtn(v,50,"No Fog: OFF",nil,function()nf=not nf if nf then l.FogEnd=1e5 else l.FogEnd=origFog end nfBtn.Text="No Fog: "..(nf and"ON"or"OFF")nfBtn.BackgroundColor3=nf and Color3.fromRGB(0,100,50)or Color3.fromRGB(60,60,70)end)

-- ESP
local espF=Instance.new("Frame",cont)espF.Size=UDim2.new(1,0,1,0)espF.BackgroundTransparency=1 espF.Visible=false
...
-- Aimbot
local aF=Instance.new("Frame",cont)aF.Size=UDim2.new(1,0,1,0)aF.BackgroundTransparency=1 aF.Visible=false
local aimOn=false local aimBtn=novobtn(aF,10,"Aimbot: OFF",nil,function()aimOn=not aimOn aimBtn.Text="Aimbot: "..(aimOn and"ON"or"OFF")aimBtn.BackgroundColor3=aimOn and Color3.fromRGB(0,100,50)or Color3.fromRGB(60,60,70)end)
...
print("Shinka Hub + Minimizar carregado! Botão SH para reabrir.")
